...
public actor LLama {
    ...
    public init(modelPath: String, contextSize: Int) throws {
        self.modelPath = modelPath
        self.model = try LLamaModel(modelPath: modelPath, contextSize: contextSize)
        ...
        vocab = llama_model_get_vocab(model.vocab)
    }
    ...
    public func getModelName() -> String {
        return URL(filePath: modelPath).lastPathComponent
    }
    
    /// Reset the context with the same loaded model before start a new chat
    private func resetContext() throws {
        ...
        logger.debug("Using \(n_threads) threads")
        
        var ctxParams = llama_context_default_params()
        ctxParams.n_ctx = nCtx
        ctxParams.n_threads = Int32(n_threads)
        ctxParams.n_threads_batch = Int32(n_threads)

        guard let newCtx = llama_init_from_model(model.model, ctxParams) else {
            throw InferError(message: "Failed to reinitialize context", code: .failedToInitializeContext)
        }
    ...
    ///   - maxGeneratedTokens: The maximum number of tokens to generate. Defaults to 128.
    ///
    /// - Returns: An `AsyncThrowingStream` emitting generated tokens as strings.
    public func infer(prompt: String, maxGeneratedTokens: Int32 = 128) -> AsyncThrowingStream<String, Error> {
        return AsyncThrowingStream { continuation in
            Task {
                do {
                    try await self.infer(prompt: prompt, maxGeneratedTokens: maxGeneratedTokens, continuation: continuation)
                } catch {
    ...
    /// - Parameters:
    ///   - prompt: The input text prompt to generate completions for.
    ///   - maxGeneratedTokens: The maximum number of tokens to generate.
    ...
    private func infer(
        ...
        var isDone = false
        let tokenList = tokenize(text: prompt, add_bos: true)
        let nCtx = llama_n_ctx(model.context)
        let nLen = min(Int32(tokenList.count) + maxGeneratedTokens, Int32(nCtx))
        var nCur: Int32 = 0
        var nDecode: Int32 = 0
        var batch = llama_batch_init(Int32(tokenList.count) + maxGeneratedTokens, 0, 1)
    ...
    private func initializeInference(
        tokenList: [llama_token],
        batch: inout llama_batch,
        nLen: Int32,
        nCur: inout Int32
    ) throws {
        let nCtx = llama_n_ctx(model.context)
        let nKvReq = tokenList.count + Int(nLen - Int32(tokenList.count))
        ...
        for (i, token) in tokenList.enumerated() {
...
//        
//        let newTokens = tokenize(text: prompt, add_bos: promptTokens.isEmpty) // only add BOS once
//        promptTokens += newTokens
//        
//        let nCtx = llama_n_ctx(model.context)
//        let nKvReq = promptTokens.count + Int(nLen - Int32(newTokens.count))
//        
//        logger.debug("\nn_len = \(nLen), n_ctx = \(nCtx), n_kv_req = \(nKvReq)")
//        
//        if nKvReq > nCtx {
...
//        batch.clear()
//        
//        let startPos = Int32(nCur) // Record the stop position from last round
//        for (i, token) in newTokens.enumerated() {
//            llamaBatchAdd(&batch, token, startPos + Int32(i), [0], false)
//        }
//        if batch.n_tokens > 0 {
//            batch.logits[Int(batch.n_tokens) - 1] = 1  // true
//        }
...
//    }
    ...
    private func generateNextToken(
        ...
        if llama_vocab_is_eog(model.vocab, newTokenID) || nCur == nLen {
            isDone = true
